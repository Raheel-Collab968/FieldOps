...
const page = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setCounts({
        total: result.data.counts.total,
        pendingJob: result.data.counts.pendingJob,
  ...
  const stats = [
    { label: 'Total Jobs', value: counts?.total ?? 0 },
    { label: 'Pending Job ', value: counts?.pendingJob ?? 0 },
    { label: 'Working Job', value: counts?.in_Progress ?? 0 },
    { label: 'Completed Job', value: counts?.completed ?? 0 },
  ];
